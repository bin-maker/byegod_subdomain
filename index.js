...
const client = redis.createClient(12345678, '127.0.0.1'); //连接你自己的redis服务器，redis 用来临时存放 work.sh 查询的数据,如果不想用可以使用mysql,甚至保存到本地txt的方式来做存取
client.auth('xxxxxx')  //redis 密码
client.on('connect', () => {
...
//网站https证书，要是http可以忽略

var options = {
    key: fs.readFileSync('./sub.key'),
    cert: fs.readFileSync('./sub.crt')
};

//以下一大段都是判断格式用的，可以不修改直接用
...
function DomainParser(domainName) {
    ...
    DomainParser.prototype.parse = function () {
        if (!input || input.length == 0) {
            failMessage('请填写域名，例如：baidu.com');
            return;
        }
    ...
    // 支持的后缀，顶级后缀，可以自己添加，主要是为了防止用户传进来垃圾乱输入的域名占用系统资源

    var one_top_level = ['cloud','tech','store','ac', 'ad', 'ae', 'aero', 'af', 'ag', 'ai', 'al', 'am', 'an', 'ao', 'aq',
        ...
        'xn--yfro4i67o', '公司', '网络', '網絡', 'xn--55qx5d', 'xn--io0a7i', 'moe', "vip", "top"
    ];

    // 支持的二级后缀，可以自己添加，主要是为了防止用户传进来垃圾乱输入的域名占用系统资源
...
let server = https.createServer(options, (req, res) => {
    res.writeHead(200);
    res.end("Don't support this way ! \n");
}).listen(8888);


// 这里填写你自己的网站域名，主要是为了防止其他网站嵌套调用你的网站，规定只允许自己网站的前端调用

let wss = new WebSocketServer({
    server: server,
    verifyClient: function (info) {
        console.log(info.origin);
        if (info.origin.indexOf('ruo.me') != -1) {
            return true;
        } else {
            return false;
        }
...
// 和前端websocket交互,这里可以自己修改,比如下面的 client 字符串就是我给前端websocket自定义的名字
        ...
        ws.on("message", (data) => {
            try {
                if (data.split(';')[0] == "client") {
                    ...
                    var t = data.split(';')[1].split('|')[1];
                    var d = data.split(';')[1].split('|')[0];


                    let check_domain = new DomainParser(d);
                    check_domain.parse();
                    if (check_domain.hasError()) {
                        try {
                            ws.send("域名输入错误！");
                        } catch {
                            ws.close();
                        ...
                        var dm = encodeURI(check_domain.getModifyName());
                        while (ws_num < t) {
                            ws_query(dm, ws_num);
                            ws_num++;
                        }
                    ...
                    function ws_query(domain, num) {
                        let result_data = '';
                        var cmd = exec("timeout 2 host -4 " + dist[num] + '.' + domain + " | grep 'address' | head -1 | cut -d ' ' -f4");
                        cmd.stdout.on('data', (data) => {

                            result_data = dist[num] + '|' + data.replace(/[\r\n]/g, '');
                        });

                        cmd.on('exit', (code) => {
                            try {
                                ws.send(result_data + "," + ws_total);
                                ws_total++;
                            } catch {
                                ws.close();
                            }
                ...
                } else if (data.split(';')[0] = "server") {
                    console.log(data);
                    let check_domain = new DomainParser(data.split(';')[1]);
                    check_domain.parse();
                    if (check_domain.hasError()) {
                        try {
                    ...
                    function ws_query(domain) {
                        let flag = 0;

                            // 执行 work.sh 脚本

                        var cmd = exec("./work.sh " + domain);
                        cmd.stdout.on('data', (data) => {
                            if (data.indexOf("live") != -1) {
                                flag = 1;
                            }

                        });

                            // redis 中取数据发送给前端,记得自己做筛选

                        cmd.on('exit', (code) => {
                            if (flag == 0) {
                                client.get(domain, (err, data) => {
                                    try {
                                        ws.send(data);
                                    } catch {
                                        ws.close();
